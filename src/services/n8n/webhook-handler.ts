...
import { MasterWebhookRequest, WebhookAction } from '../../types/n8n'
 ...
 */
export function validateWebhookAction(body: unknown): body is { action: string } {
 ...
 */
export function validateMasterWebhookRequest(body: unknown): body is MasterWebhookRequest {
  ...
  switch (request.action) {
    case WebhookAction.COMPANY_RESEARCH:
      return typeof request.companyId === 'string' && request.companyId.length > 0
    ...
    case WebhookAction.GENERATE_CONTENT:
      return typeof request.generatedPostId === 'string' && request.generatedPostId.length > 0
 ...
 */
export function validateWebhookAuthentication(request: NextRequest, secret?: string): boolean {
  if (!secret) {
 ...
 */
export function createNotFoundErrorResponse(resource: string, id?: string): WebhookErrorResponse {
 ...
 */
export function createAPIErrorResponse(message: string, details?: unknown): WebhookErrorResponse {
  return {
    success: false,
    error: message,
